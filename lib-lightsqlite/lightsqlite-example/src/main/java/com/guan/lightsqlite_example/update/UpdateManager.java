package com.guan.lightsqlite_example.update;
...
import com.guan.lightsqlite.update.CreateDb;
import com.guan.lightsqlite.update.CreateVersion;
import com.guan.lightsqlite.update.UpdateDb;
import com.guan.lightsqlite.update.UpdateDbXml;
import com.guan.lightsqlite.update.UpdateStep;
...
public class UpdateManager {
    ...
    private File parentFile = new File(Environment.getExternalStorageDirectory(), Constant.FILE_NAME);
    private File bakFile = new File(parentFile, Constant.BACK_DB_NAME);
    ...
    public void checkThisVersionTable(Context context) {
        ...
        userList = userDao.query(new UserBean());
        UpdateDbXml xml = readDbXml(context);

        String thisVersion = this.getLocalDbName(context);
        CreateVersion thisCreateVersion = analyseCreateVersion(xml, thisVersion);
        try {
            // 根据建表脚本,核实一遍应该存在的表，不存在时创建新表
    ...
    public void startUpdateDb(Context context) {
        ...
        if (getLocalVersionInfo()) {
            // 拿到最新版本:newVersion
            // 拿到当前版本:thisVersion
            UpdateStep updateStep = analyseUpdateStep(updateDbxml, thisVersion, newVersion);

            if (updateStep == null) {
                return;
            }
            List<UpdateDb> updateDbs = updateStep.getUpdateDbs();
            CreateVersion createVersion = analyseCreateVersion(updateDbxml, newVersion);
     ...
     * @return
     */
    private UpdateStep analyseUpdateStep(UpdateDbXml xml, String thisVersion, String newVersion) {
        if (thisVersion == null || newVersion == null) {
            return null;
        }
        ...
        for (UpdateStep step : steps) {
            if (step.getVersionFrom() == null || step.getVersionTo() == null) {
                ...
                // 升级来源以逗号分隔
                String[] localVersionArray = step.getVersionFrom().split(",");

                if (localVersionArray.length > 0) {
                    for (String aLastVersionArray : localVersionArray) {
                        // 有一个配到update节点即升级数据
                        if (thisVersion.equalsIgnoreCase(aLastVersionArray) && step.getVersionTo().equalsIgnoreCase(newVersion)) {
                            thisStep = step;
                            break;
                        }
                    }
    ...
    private CreateVersion analyseCreateVersion(UpdateDbXml xml, String version) {
        ...
        if (createVersions != null) {
            for (CreateVersion item : createVersions) {
                ...
                for (String aCreateVersion : createVersion) {
                    if (aCreateVersion.trim().equalsIgnoreCase(version)) {
                        cv = item;
                        break;
                    }
                }
     ...
     * @see
     */
    public String getLocalDbName(Context context) {
...
//        return versionName;
    ...
    public boolean saveVersionInfo(Context context, String newVersion) {
        ...
        try {
            ...
            writer.write(newVersion + INFO_FILE_DIV + getLocalDbName(context));
            writer.flush();
            ret = true;
